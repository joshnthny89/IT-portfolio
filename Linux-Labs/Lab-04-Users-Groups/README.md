...
<sub><em>Used the `dscl . list /Users` command to enumerate local user accounts, demonstrating how system administrators and security professionals can identify accounts configured on a system.</em></sub>
...
<sub><em>In the above 2 screenshots I performed user and group enumeration using the `users`, `groups`, and `dscl . list /Users` commands to review active sessions, group memberships, and user accounts configured on the macOS system. Finished the lab by using `sudo -l` to review available administrative privileges and verify the level of access assigned to the current user account.</em></sub>
